...
oc create -n ${IOT_OCP_RESOURCES_PROJECT} -f ${SCRIPT_BASE_DIR}/../../support/templates/rhel7-is.json 2>&1 | tee -a "${LOG_FILE_NAME}"
...
oc import-image -n ${IOT_OCP_RESOURCES_PROJECT} rhel7 --all=true 2>&1 | tee -a "${LOG_FILE_NAME}"
